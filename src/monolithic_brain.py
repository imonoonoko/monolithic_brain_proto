...
class ActiveInferenceController(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.register_buffer('curiosity_threshold', torch.tensor(config.CURIOSITY_THRESHOLD)) 
...
class EpisodicMemory(nn.Module):
    ...
    def recall_memory(self, query_vector: torch.Tensor, threshold: float = 0.0) -> float:
        """
...
class NeuralSymbolicBrain(nn.Module):
    ...
    """
    def __init__(self, model_path: str = config.MODEL_FILENAME, n_ctx: int = config.CTX_SIZE, **kwargs):
        super().__init__()
        ...
        self.llm_gen = Llama(model_path=model_path, n_ctx=n_ctx, embedding=False, logits_all=False, verbose=False, **kwargs)

        # Instance 2: Embedding (Right Hemisphere / Hippocampus Feed)
        print("  - Initializing Embedder (Right Hemisphere)...")
        self.llm_embed = Llama(model_path=model_path, n_ctx=n_ctx, embedding=True, logits_all=False, verbose=False, **kwargs)
        
        # Inspect embedding size
        try:
            dummy_embed = self.llm_embed.create_embedding("Init")['data'][0]['embedding']
            embed_dim = len(dummy_embed)
            if embed_dim < 16:
                 print(f"Warning: Detected embedding dimension {embed_dim} looks suspicious. Fallback to {config.EMBED_DIM_DETECT}.")
                 embed_dim = config.EMBED_DIM_DETECT
        except Exception as e:
            print(f"Warning: Could not detect embedding dimension ({e}). Fallback to {config.EMBED_DIM_DETECT}.")
            embed_dim = config.EMBED_DIM_DETECT
    ...
    def forward(self, prompt, max_tokens=64):
        ...
        current_embedding = embed_resp['data'][0]['embedding'] # List[float] OR List[List[float]]
        ...
        if embed_tensor.dim() == 2:
            ...
            embed_tensor = torch.mean(embed_tensor, dim=0, keepdim=True) # (1, embed_dim)
